...
char *progname = "";            // HACK: progname may be relative path to component if called from other nonpath place.
char *version = "0.2-alpha";
...
void print_version ()
{
  ...
  printf ("This is free software; See LICENSE for license details.\n");
}
  ...
  while (1)
	...
	case 't':		/* cell type */
	  if (set_type (optarg) != 0)
	    {
	      fprintf (stderr, "%s: bad cell type %s\n", progname, optarg); //STDERR wont get redirected anyway. To replace with printf.
	      exit (EXIT_FAILURE);
	    }
	  break;
